...
func MakeBlockContext(s storage.Storage, client chain.Client, oracle *oracle.Oracle, config *common.Config, accounts *storage.AccountsMap) *blockContext {
	var bc blockContext
	bc.Storage = s
	bc.Batch = s.NewBatch()
	bc.Config = config
...
func (bc *blockContext) process(bd *chain.BlockData, stats *chain.Stats) (dags_count, trx_count uint64, err error) {
	...
	r.Process(totalReward, bc.Block.Dags, bc.Block.Transactions, bc.Block.Votes, bc.Block.Pbft.Author)
